...
* @details
* Inicie: Inicializa el cuerpo con carga, radio, posición y velocidad.
* GetF: Devuelve la fuerza neta actuando sobre el cuerpo.
* BorreFuerza: Resetea la fuerza neta a cero.
* AdicioneFuerzaElectrica: Calcula y añade la fuerza eléctrica de otro cuerpo.
...
* GetQ: Devuelve la carga del cuerpo.
...
*/
public:
    void Inicie(double q0, double R0, const Vector3d &r0, const Vector3d &V0);
    Vector3d GetF() const { return F; }
    void BorreFuerza();
    void AdicioneFuerzaElectrica(Cuerpo &otro);
